...
public class AllActivity extends BaseActivity implements View.OnClickListener {
    ...
    /**
     *  Print all ESC commands supported by starPOS printer
     *  To verify that the printer works ok
     */
    public void onPrint(View view) {
        byte[] rv = BytesUtil.customData();
        Bitmap head = BitmapFactory.decodeResource(getResources(), R.mipmap.ic_launcher);
        //打印光栅位图(print raster bitmap ——normal)
        rv = BytesUtil.byteMerger(rv, ESCUtil.printRasterBitmap(head, 0));
        //打印光栅位图  倍宽  (print raster bitmap ——double width)
        rv = BytesUtil.byteMerger(rv,ESCUtil.printRasterBitmap(head, 1));
        //打印光栅位图  倍高    (print raster bitmap ——double height)
        rv = BytesUtil.byteMerger(rv,ESCUtil.printRasterBitmap(head, 2));
        //打印光栅位图  倍宽倍高   (print raster bitmap ——double width and height)
        rv = BytesUtil.byteMerger(rv,ESCUtil.printRasterBitmap(head, 3));
        ...
        if(BluetoothUtil.isBlueToothPrinter){
            ...
            starPOSPrintHelper.getInstance().sendRawData(rv);
        }
    }
